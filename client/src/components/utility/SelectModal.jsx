...
const SelectModal = ({ handleClose, handleSelect, options, selectOption }) => {
    ...
    const handleCheckboxChange = (option, checked) => {
        if (selectOption === "multiple") {
            ...
            if (checked) {
                setTempSelectedOptions((prevSelected) => [...prevSelected, option.name]);
            } else {
        ...
        <div className="custom-modal-overlay">
            <div className="custom-modal">
                <div className="custom-modal-header">
                    ...
                    <button className="close-btn" onClick={handleClose}><X className="text-gray-700" /></button>
                </div>
                        ...
                        {options.map((option, index) => (
                            <li key={index} className="flex items-center space-x-3">
                                <button
                                    ...
                                    onClick={() => handleCheckboxChange(option, !(selectOption=="single"?tempSelectedOptions==option:tempSelectedOptions.includes(option.name)))}
                                >
                ...
                <div className="custom-modal-footer">
                    ...
                    <button className="select-btn" onClick={handleSelectClick}>Select</button>
                </div>
            </div>
        </div>
    );
