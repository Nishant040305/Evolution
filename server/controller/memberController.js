...
const inviteMember = async (req, res) => {
    try {
        ...
        const project = await Project.findById(id).populate('user members.user description name');
        if (!project) {
        ...
        res.status(200).json({ message: 'User invited successfully',Notification:NotificationInvite });
    } catch (error) {
...
const acceptCollaboration = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Error accepting collaboration', error });
    }
};
module.exports = {
    inviteMember,
